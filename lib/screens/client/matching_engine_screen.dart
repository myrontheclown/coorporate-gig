...
class _MatchingEngineScreenState extends State<MatchingEngineScreen> {
  ...
  void initState() {
    super.initState();
    // Only on-duty workers are candidates for matching
    _matches = MockData.onDutyWorkers.where((w) =>
        w.profession.toLowerCase() ==
        widget.worker.profession.toLowerCase()).toList();
    // If no on-duty workers match the profession, take the first few on-duty workers
    if (_matches.isEmpty) _matches = [...MockData.onDutyWorkers].take(3).toList();
    // Last-resort fallback: if zero on-duty workers exist at all, use the
    // originally requested worker so the flow never gets stuck
    if (_matches.isEmpty) _matches = [widget.worker];
    _runMatching();
  }
